...
extern Vector_msg vector_receive_msg;
Vector_msg vector_upload_msg;
...
void chassis_task(void const *pvParameters){
    ...
    while (1){
        ...
        chassis_Vector_Upload();

//        chassis_ctrl_info_get();

        chassis_vector_info_get();//云台主控下发信息接收的解算函数
...
static void chassis_ctrl_info_get(){
    ...
    chassis.vw=(float)(rc_ctrl.rc.ch[CHASSIS_Z_CHANNEL])*RC_TO_VW
               +chassis.vw_pc;
...
static void chassis_back_handle()
{
    if(chassis.absolute_angle_get<=7 && chassis.absolute_angle_get>=-7 && (ABS(motor_LF_speed+motor_RF_speed)/2.0f)<6 )
    {
...
//            give_current_limit[i]=(int16_t)(chassis.motor_chassis[i].give_current*limit_k);
    ...
    {
        for(uint8_t i=0;i<4;i++)
            give_current_limit[i]=(int16_t)(chassis.motor_chassis[i].give_current*chassis.chassis_power_limit.total_current_limit
...
static void chassis_angle_update(){
    ...
    chassis.yaw_relative_angle_get=vector_receive_msg.yaw_relative_angle_get.value;
}
...
void chassis_Vector_Download(){
//上板要发送给下板的速度向量。

    chassis_vector_msg.vx.value=chassis.vx;
    chassis_vector_msg.vw.value=chassis.vw;
    chassis_vector_msg.mode.value=chassis.mode;
    CAN_msg_send(CAN_1, CAN_VECTOR_XYZ_0X100_ID, chassis_vector_msg);

}

static void chassis_vector_info_get(){
    chassis.vx=vector_receive_msg.vx.value;
    chassis.vw=vector_receive_msg.vw.value;
    chassis.run=vector_receive_msg.run.value;
    chassis.mode=vector_receive_msg.mode.value;
    VAL_LIMIT(chassis.vx,-MAX_CHASSIS_VX_SPEED,MAX_CHASSIS_VX_SPEED);
...
static void chassis_push_control() {
    ...
    chassis.motor_push.absolute_angle_get = motor_ecd_to_angle_change(chassis.motor_push.motor_measure->ecd,
                                                                      chassis.motor_push.motor_measure->offset_ecd);//动量快相对角度计算
    chassis.motor_left.absolute_angle_get = motor_ecd_to_angle_change(chassis.motor_left.motor_measure->ecd,
                                                                      chassis.motor_left.motor_measure->offset_ecd);


    speed_erro = chassis.vx - chassis_real_speed;//速度补偿值


    if (speed_erro > -2 && speed_erro < 2)   //速度差控制
    {

        chassis.motor_push.absolute_angle_set = (float) (0);
        chassis.motor_left.absolute_angle_set = (float) (0);
    }


//    else if (speed_erro >= 2) {
//
//        chassis.motor_push.absolute_angle_set = (float) ((speed_erro-2) * 35);
//        chassis.motor_left.absolute_angle_set = (float) ((-speed_erro+2) * 33.5);
//    } else if (speed_erro <= -2) {
//
//        chassis.motor_push.absolute_angle_set = (float) ((speed_erro+2) * 36.2);
//        chassis.motor_left.absolute_angle_set = (float) ((-speed_erro-2) * 41);
//    }



        chassis.motor_push.absolute_angle_set = (float) ((-speed_erro) * 35);
        chassis.motor_left.absolute_angle_set = (float) ((speed_erro) * 35);
    ...
    if (chassis.motor_push.absolute_angle_set > 165)  // 设置限幅
    {
        chassis.motor_push.absolute_angle_set = 165;

    } else if (chassis.motor_push.absolute_angle_set < -165) {

        chassis.motor_push.absolute_angle_set = -165;
    }

    if (chassis.motor_push.absolute_angle_get > 165)//获取限幅
    {
        chassis.motor_push.absolute_angle_get = 165;

    } else if (chassis.motor_push.absolute_angle_get < -165) {

        chassis.motor_push.absolute_angle_get = -165;
    }


    if (chassis.motor_left.absolute_angle_set > 165)  // 设置限幅
    {
        chassis.motor_left.absolute_angle_set = 165;

    } else if (chassis.motor_left.absolute_angle_set < -165) {

        chassis.motor_left.absolute_angle_set = -165;
    }

    if (chassis.motor_left.absolute_angle_get > 165)//获取限幅
    {
        chassis.motor_left.absolute_angle_get = 165;

    } else if (chassis.motor_left.absolute_angle_get < -165) {

        chassis.motor_left.absolute_angle_get = -165;
    }

    //串级pid
    push_speed_out = pid_calc(&chassis.motor_push.angle_p,
                              chassis.motor_push.absolute_angle_get,
                              chassis.motor_push.absolute_angle_set);
    left_speed_out = pid_calc(&chassis.motor_left.angle_p,
                              chassis.motor_left.absolute_angle_get,
                              chassis.motor_left.absolute_angle_set);
    chassis.motor_push.gyro_set = (float) (push_speed_out / push_rate);
    chassis.motor_left.gyro_set = (float) (left_speed_out / push_rate);


    chassis.motor_push.give_current = pid_calc(&chassis.motor_push.speed_p,
                                               chassis.motor_push.motor_measure->speed_rpm,
                                               chassis.motor_push.gyro_set);
    chassis.motor_left.give_current = pid_calc(&chassis.motor_left.speed_p,
                                               chassis.motor_left.motor_measure->speed_rpm,
                                               chassis.motor_left.gyro_set);


    CAN_cmd_motor(CAN_2, 0x1FF, chassis.motor_push.give_current, chassis.motor_left.give_current,
                  0, 0);
//    CAN_cmd_motor(CAN_2, 0x1FF, 0, 0,
//                  0, 0);
}

static void chassis_Vector_Upload(){
    vector_upload_msg.angle.value=chassis.absolute_angle_get;
    CAN_msg_send(CAN_1, CAN_CHASSIS_ANGLE_0x103_ID, vector_upload_msg);
}

static void chassis_off_ground_detection(){
    if(tof_msg.distance.value>=175 || tof_msg.dis_status.value!=0){
        chassis.mode=CHASSIS_OFF_GROUND;
    }
}